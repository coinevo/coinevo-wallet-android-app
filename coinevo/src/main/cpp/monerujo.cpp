...
JNIEXPORT jint JNICALL JNI_OnLoad(JavaVM *jvm, void *reserved) {
    ...
    class_TransactionInfo = static_cast<jclass>(jenv->NewGlobalRef(
            jenv->FindClass("io/wookey/coinevo/model/TransactionInfo")));
    class_Transfer = static_cast<jclass>(jenv->NewGlobalRef(
            jenv->FindClass("io/wookey/coinevo/model/Transfer")));
    class_WalletListener = static_cast<jclass>(jenv->NewGlobalRef(
            jenv->FindClass("io/wookey/coinevo/model/WalletListener")));
    class_Ledger = static_cast<jclass>(jenv->NewGlobalRef(
            jenv->FindClass("io/wookey/coinevo/ledger/Ledger")));
    class_SubaddressRow = static_cast<jclass>(jenv->NewGlobalRef(
            jenv->FindClass("io/wookey/coinevo/model/SubaddressRow")));
...
struct MyWalletListener : Coinevo::WalletListener {
    jobject jlistener;
...
Java_io_wookey_coinevo_model_WalletManager_createWalletJ(JNIEnv *env, jobject instance,
    ...
    Coinevo::NetworkType _networkType = static_cast<Coinevo::NetworkType>(networkType);

    Coinevo::Wallet *wallet =
            Coinevo::WalletManagerFactory::getWalletManager()->createWallet(
                    std::string(_path),
...
JNIEXPORT jlong JNICALL
Java_io_wookey_coinevo_model_WalletManager_openWalletJ(JNIEnv *env, jobject instance,
                                                      jstring path, jstring password,
                                                      jint networkType) {
    const char *_path = env->GetStringUTFChars(path, NULL);
    const char *_password = env->GetStringUTFChars(password, NULL);
    ...
    Coinevo::Wallet *wallet =
            Coinevo::WalletManagerFactory::getWalletManager()->openWallet(
                    std::string(_path),
                    std::string(_password),
                    _networkType);
...
JNIEXPORT jlong JNICALL
Java_io_wookey_coinevo_model_WalletManager_recoveryWalletJ(JNIEnv *env, jobject instance,
                                                          jstring path, jstring password,
                                                          jstring mnemonic,
                                                          jint networkType,
                                                          jlong restoreHeight) {
    ...
    Coinevo::Wallet *wallet =
            Coinevo::WalletManagerFactory::getWalletManager()->recoveryWallet(
                    std::string(_path),
                    std::string(_password),
...
JNIEXPORT jlong JNICALL
Java_io_wookey_coinevo_model_WalletManager_createWalletFromKeysJ(JNIEnv *env, jobject instance,
                                                                jstring path, jstring password,
                                                                jstring language,
                                                                jint networkType,
                                                                jlong restoreHeight,
    ...
    Coinevo::Wallet *wallet =
            Coinevo::WalletManagerFactory::getWalletManager()->createWalletFromKeys(
                    std::string(_path),
                    std::string(_password),
                    std::string(_language),
                    _networkType,
...
JNIEXPORT jlong JNICALL
Java_io_wookey_coinevo_model_WalletManager_createWalletFromDeviceJ(JNIEnv *env, jobject instance,
                                                                  jstring path,
                                                                  jstring password,
                                                                  jint networkType,
                                                                  jstring deviceName,
    ...
    Coinevo::Wallet *wallet =
            Coinevo::WalletManagerFactory::getWalletManager()->createWalletFromDevice(
                    std::string(_path),
                    std::string(_password),
                    _networkType,
...
Java_io_wookey_coinevo_model_WalletManager_walletExists(JNIEnv *env, jobject instance,
                                                       jstring path) {
    const char *_path = env->GetStringUTFChars(path, NULL);
    bool exists =
            Coinevo::WalletManagerFactory::getWalletManager()->walletExists(std::string(_path));
    env->ReleaseStringUTFChars(path, _path);
    return static_cast<jboolean>(exists);
}

JNIEXPORT jboolean JNICALL
Java_io_wookey_coinevo_model_WalletManager_verifyWalletPassword(JNIEnv *env, jobject instance,
    ...
    const char *_password = env->GetStringUTFChars(password, NULL);
    bool passwordOk =
            Coinevo::WalletManagerFactory::getWalletManager()->verifyWalletPassword(
...
Java_io_wookey_coinevo_model_WalletManager_queryWalletDeviceJ(JNIEnv *env, jobject instance,
                                                             jstring keys_file_name,
                                                             jstring password) {
    const char *_keys_file_name = env->GetStringUTFChars(keys_file_name, NULL);
    const char *_password = env->GetStringUTFChars(password, NULL);
    Coinevo::Wallet::Device device_type;
    bool ok = Coinevo::WalletManagerFactory::getWalletManager()->
...
Java_io_wookey_coinevo_model_WalletManager_findWallets(JNIEnv *env, jobject instance,
                                                      jstring path) {
    const char *_path = env->GetStringUTFChars(path, NULL);
    std::vector<std::string> walletPaths =
            Coinevo::WalletManagerFactory::getWalletManager()->findWallets(std::string(_path));
    env->ReleaseStringUTFChars(path, _path);
    return cpp2java(env, walletPaths);
}

JNIEXPORT jstring JNICALL
Java_io_wookey_coinevo_model_WalletManager_getErrorString(JNIEnv *env, jobject instance) {
    Coinevo::Wallet *wallet = getHandle<Coinevo::Wallet>(env, instance);
...
Java_io_wookey_coinevo_model_WalletManager_setDaemonAddressJ(JNIEnv *env, jobject instance,
                                                            jstring address) {
    const char *_address = env->GetStringUTFChars(address, NULL);
    Coinevo::WalletManagerFactory::getWalletManager()->setDaemonAddress(std::string(_address));
    env->ReleaseStringUTFChars(address, _address);
}

// returns whether the daemon can be reached, and its version number
JNIEXPORT jint JNICALL
Java_io_wookey_coinevo_model_WalletManager_getDaemonVersion(JNIEnv *env,
                                                           jobject instance) {
    uint32_t version;
    bool isConnected =
            Coinevo::WalletManagerFactory::getWalletManager()->connected(&version);
    if (!isConnected) version = 0;
    return version;
}

JNIEXPORT jlong JNICALL
Java_io_wookey_coinevo_model_WalletManager_getBlockchainHeight(JNIEnv *env, jobject instance) {
    return Coinevo::WalletManagerFactory::getWalletManager()->blockchainHeight();
}

JNIEXPORT jlong JNICALL
Java_io_wookey_coinevo_model_WalletManager_getBlockchainTargetHeight(JNIEnv *env,
                                                                    jobject instance) {
    return Coinevo::WalletManagerFactory::getWalletManager()->blockchainTargetHeight();
}

JNIEXPORT jlong JNICALL
Java_io_wookey_coinevo_model_WalletManager_getNetworkDifficulty(JNIEnv *env, jobject instance) {
    return Coinevo::WalletManagerFactory::getWalletManager()->networkDifficulty();
}

JNIEXPORT jdouble JNICALL
Java_io_wookey_coinevo_model_WalletManager_getMiningHashRate(JNIEnv *env, jobject instance) {
    return Coinevo::WalletManagerFactory::getWalletManager()->miningHashRate();
}

JNIEXPORT jlong JNICALL
Java_io_wookey_coinevo_model_WalletManager_getBlockTarget(JNIEnv *env, jobject instance) {
    return Coinevo::WalletManagerFactory::getWalletManager()->blockTarget();
}

JNIEXPORT jboolean JNICALL
Java_io_wookey_coinevo_model_WalletManager_isMining(JNIEnv *env, jobject instance) {
    return static_cast<jboolean>(Coinevo::WalletManagerFactory::getWalletManager()->isMining());
}

JNIEXPORT jboolean JNICALL
Java_io_wookey_coinevo_model_WalletManager_startMining(JNIEnv *env, jobject instance,
                                                      jstring address,
                                                      jboolean background_mining,
                                                      jboolean ignore_battery) {
    const char *_address = env->GetStringUTFChars(address, NULL);
    bool success =
            Coinevo::WalletManagerFactory::getWalletManager()->startMining(std::string(_address),
                                                                             background_mining,
                                                                             ignore_battery);
    env->ReleaseStringUTFChars(address, _address);
...
JNIEXPORT jboolean JNICALL
Java_io_wookey_coinevo_model_WalletManager_stopMining(JNIEnv *env, jobject instance) {
    return static_cast<jboolean>(Coinevo::WalletManagerFactory::getWalletManager()->stopMining());
}

JNIEXPORT jstring JNICALL
Java_io_wookey_coinevo_model_WalletManager_resolveOpenAlias(JNIEnv *env, jobject instance,
                                                           jstring address,
                                                           jboolean dnssec_valid) {
    const char *_address = env->GetStringUTFChars(address, NULL);
    bool _dnssec_valid = (bool) dnssec_valid;
    std::string resolvedAlias =
            Coinevo::WalletManagerFactory::getWalletManager()->resolveOpenAlias(
                    std::string(_address),
                    _dnssec_valid);
    env->ReleaseStringUTFChars(address, _address);
...
JNIEXPORT jboolean JNICALL
Java_io_wookey_coinevo_model_WalletManager_closeJ(JNIEnv *env, jobject instance,
                                                 jobject walletInstance) {
    Coinevo::Wallet *wallet = getHandle<Coinevo::Wallet>(env, walletInstance);
    bool closeSuccess = Coinevo::WalletManagerFactory::getWalletManager()->closeWallet(wallet,
                                                                                         false);
    if (closeSuccess) {
...
JNIEXPORT jstring JNICALL
Java_io_wookey_coinevo_model_Wallet_getSeed(JNIEnv *env, jobject instance) {
    Coinevo::Wallet *wallet = getHandle<Coinevo::Wallet>(env, instance);
    return env->NewStringUTF(wallet->seed().c_str());
}

JNIEXPORT jstring JNICALL
Java_io_wookey_coinevo_model_Wallet_getSeedLanguage(JNIEnv *env, jobject instance) {
    Coinevo::Wallet *wallet = getHandle<Coinevo::Wallet>(env, instance);
    return env->NewStringUTF(wallet->getSeedLanguage().c_str());
}

JNIEXPORT void JNICALL
Java_io_wookey_coinevo_model_Wallet_setSeedLanguage(JNIEnv *env, jobject instance,
                                                   jstring language) {
    const char *_language = env->GetStringUTFChars(language, NULL);
    Coinevo::Wallet *wallet = getHandle<Coinevo::Wallet>(env, instance);
...
JNIEXPORT jint JNICALL
Java_io_wookey_coinevo_model_Wallet_getStatusJ(JNIEnv *env, jobject instance) {
    Coinevo::Wallet *wallet = getHandle<Coinevo::Wallet>(env, instance);
    return wallet->status();
}

JNIEXPORT jstring JNICALL
Java_io_wookey_coinevo_model_Wallet_getErrorString(JNIEnv *env, jobject instance) {
    Coinevo::Wallet *wallet = getHandle<Coinevo::Wallet>(env, instance);
    return env->NewStringUTF(wallet->errorString().c_str());
}

JNIEXPORT jboolean JNICALL
Java_io_wookey_coinevo_model_Wallet_setPassword(JNIEnv *env, jobject instance,
                                               jstring password) {
    const char *_password = env->GetStringUTFChars(password, NULL);
    Coinevo::Wallet *wallet = getHandle<Coinevo::Wallet>(env, instance);
...
JNIEXPORT jstring JNICALL
Java_io_wookey_coinevo_model_Wallet_getAddressJ(JNIEnv *env, jobject instance,
                                               jint accountIndex,
                                               jint addressIndex) {
    Coinevo::Wallet *wallet = getHandle<Coinevo::Wallet>(env, instance);
...
JNIEXPORT jstring JNICALL
Java_io_wookey_coinevo_model_Wallet_getPath(JNIEnv *env, jobject instance) {
    Coinevo::Wallet *wallet = getHandle<Coinevo::Wallet>(env, instance);
    return env->NewStringUTF(wallet->path().c_str());
}

JNIEXPORT jint JNICALL
Java_io_wookey_coinevo_model_Wallet_nettype(JNIEnv *env, jobject instance) {
    Coinevo::Wallet *wallet = getHandle<Coinevo::Wallet>(env, instance);
...
JNIEXPORT jstring JNICALL
Java_io_wookey_coinevo_model_Wallet_getIntegratedAddress(JNIEnv *env, jobject instance,
                                                        jstring payment_id) {
...
JNIEXPORT jstring JNICALL
Java_io_wookey_coinevo_model_Wallet_getSecretViewKey(JNIEnv *env, jobject instance) {
    Coinevo::Wallet *wallet = getHandle<Coinevo::Wallet>(env, instance);
    return env->NewStringUTF(wallet->secretViewKey().c_str());
}

JNIEXPORT jstring JNICALL
Java_io_wookey_coinevo_model_Wallet_getSecretSpendKey(JNIEnv *env, jobject instance) {
    Coinevo::Wallet *wallet = getHandle<Coinevo::Wallet>(env, instance);
    return env->NewStringUTF(wallet->secretSpendKey().c_str());
}

JNIEXPORT jstring JNICALL
Java_io_wookey_coinevo_model_Wallet_getPublicViewKey(JNIEnv *env, jobject instance) {
    Coinevo::Wallet *wallet = getHandle<Coinevo::Wallet>(env, instance);
    return env->NewStringUTF(wallet->publicViewKey().c_str());
}

JNIEXPORT jstring JNICALL
Java_io_wookey_coinevo_model_Wallet_getPublicSpendKey(JNIEnv *env, jobject instance) {
    Coinevo::Wallet *wallet = getHandle<Coinevo::Wallet>(env, instance);
    return env->NewStringUTF(wallet->publicSpendKey().c_str());
}

JNIEXPORT jboolean JNICALL
Java_io_wookey_coinevo_model_Wallet_store(JNIEnv *env, jobject instance,
                                         jstring path) {
    const char *_path = env->GetStringUTFChars(path, NULL);
    Coinevo::Wallet *wallet = getHandle<Coinevo::Wallet>(env, instance);
...
JNIEXPORT jstring JNICALL
Java_io_wookey_coinevo_model_Wallet_getFilename(JNIEnv *env, jobject instance) {
    Coinevo::Wallet *wallet = getHandle<Coinevo::Wallet>(env, instance);
...
JNIEXPORT jboolean JNICALL
Java_io_wookey_coinevo_model_Wallet_initJ(JNIEnv *env, jobject instance,
...
JNIEXPORT void JNICALL
Java_io_wookey_coinevo_model_Wallet_setRestoreHeight(JNIEnv *env, jobject instance,
                                                    jlong height) {
    Coinevo::Wallet *wallet = getHandle<Coinevo::Wallet>(env, instance);
...
JNIEXPORT jlong JNICALL
Java_io_wookey_coinevo_model_Wallet_getRestoreHeight(JNIEnv *env, jobject instance) {
    Coinevo::Wallet *wallet = getHandle<Coinevo::Wallet>(env, instance);
...
JNIEXPORT jint JNICALL
Java_io_wookey_coinevo_model_Wallet_getConnectionStatusJ(JNIEnv *env, jobject instance) {
    Coinevo::Wallet *wallet = getHandle<Coinevo::Wallet>(env, instance);
...
JNIEXPORT jlong JNICALL
Java_io_wookey_coinevo_model_Wallet_getBalance(JNIEnv *env, jobject instance,
                                              jint accountIndex) {
    Coinevo::Wallet *wallet = getHandle<Coinevo::Wallet>(env, instance);
...
JNIEXPORT jlong JNICALL
Java_io_wookey_coinevo_model_Wallet_getBalanceAll(JNIEnv *env, jobject instance) {
    Coinevo::Wallet *wallet = getHandle<Coinevo::Wallet>(env, instance);
    return wallet->balanceAll();
}

JNIEXPORT jlong JNICALL
Java_io_wookey_coinevo_model_Wallet_getUnlockedBalance(JNIEnv *env, jobject instance,
                                                      jint accountIndex) {
    Coinevo::Wallet *wallet = getHandle<Coinevo::Wallet>(env, instance);
...
JNIEXPORT jlong JNICALL
Java_io_wookey_coinevo_model_Wallet_getUnlockedBalanceAll(JNIEnv *env, jobject instance) {
    Coinevo::Wallet *wallet = getHandle<Coinevo::Wallet>(env, instance);
    return wallet->unlockedBalanceAll();
}

JNIEXPORT jboolean JNICALL
Java_io_wookey_coinevo_model_Wallet_isWatchOnly(JNIEnv *env, jobject instance) {
    Coinevo::Wallet *wallet = getHandle<Coinevo::Wallet>(env, instance);
    return static_cast<jboolean>(wallet->watchOnly());
}

JNIEXPORT jlong JNICALL
Java_io_wookey_coinevo_model_Wallet_getBlockChainHeight(JNIEnv *env, jobject instance) {
    Coinevo::Wallet *wallet = getHandle<Coinevo::Wallet>(env, instance);
    return wallet->blockChainHeight();
}

JNIEXPORT jlong JNICALL
Java_io_wookey_coinevo_model_Wallet_getApproximateBlockChainHeight(JNIEnv *env,
                                                                  jobject instance) {
    Coinevo::Wallet *wallet = getHandle<Coinevo::Wallet>(env, instance);
...
JNIEXPORT jlong JNICALL
Java_io_wookey_coinevo_model_Wallet_getDaemonBlockChainHeight(JNIEnv *env, jobject instance) {
    Coinevo::Wallet *wallet = getHandle<Coinevo::Wallet>(env, instance);
    return wallet->daemonBlockChainHeight();
}

JNIEXPORT jlong JNICALL
Java_io_wookey_coinevo_model_Wallet_getDaemonBlockChainTargetHeight(JNIEnv *env,
                                                                   jobject instance) {
    Coinevo::Wallet *wallet = getHandle<Coinevo::Wallet>(env, instance);
...
JNIEXPORT jboolean JNICALL
Java_io_wookey_coinevo_model_Wallet_isSynchronized(JNIEnv *env, jobject instance) {
    Coinevo::Wallet *wallet = getHandle<Coinevo::Wallet>(env, instance);
    return static_cast<jboolean>(wallet->synchronized());
}

JNIEXPORT jint JNICALL
Java_io_wookey_coinevo_model_Wallet_getDeviceTypeJ(JNIEnv *env, jobject instance) {
    Coinevo::Wallet *wallet = getHandle<Coinevo::Wallet>(env, instance);
    Coinevo::Wallet::Device device_type = wallet->getDeviceType();
    return static_cast<jint>(device_type);
}

//void cn_slow_hash(const void *data, size_t length, char *hash); // from crypto/hash-ops.h
//JNIEXPORT jbyteArray JNICALL
//Java_io_wookey_coinevo_util_KeyStoreHelper_slowHash(JNIEnv *env, jclass clazz,
...
JNIEXPORT jstring JNICALL
Java_io_wookey_coinevo_model_Wallet_getDisplayAmount(JNIEnv *env, jclass clazz,
                                                    jlong amount) {
    return env->NewStringUTF(Coinevo::Wallet::displayAmount(amount).c_str());
}

JNIEXPORT jlong JNICALL
Java_io_wookey_coinevo_model_Wallet_getAmountFromString(JNIEnv *env, jclass clazz,
                                                       jstring amount) {
    const char *_amount = env->GetStringUTFChars(amount, NULL);
    uint64_t x = Coinevo::Wallet::amountFromString(_amount);
    env->ReleaseStringUTFChars(amount, _amount);
    return x;
}

JNIEXPORT jlong JNICALL
Java_io_wookey_coinevo_model_Wallet_getAmountFromDouble(JNIEnv *env, jclass clazz,
                                                       jdouble amount) {
    return Coinevo::Wallet::amountFromDouble(amount);
}

JNIEXPORT jstring JNICALL
Java_io_wookey_coinevo_model_Wallet_generatePaymentId(JNIEnv *env, jclass clazz) {
    return env->NewStringUTF(Coinevo::Wallet::genPaymentId().c_str());
}

JNIEXPORT jboolean JNICALL
Java_io_wookey_coinevo_model_Wallet_isPaymentIdValid(JNIEnv *env, jclass clazz,
                                                    jstring payment_id) {
    const char *_payment_id = env->GetStringUTFChars(payment_id, NULL);
    bool isValid = Coinevo::Wallet::paymentIdValid(_payment_id);
    env->ReleaseStringUTFChars(payment_id, _payment_id);
    return static_cast<jboolean>(isValid);
}

JNIEXPORT jboolean JNICALL
Java_io_wookey_coinevo_model_Wallet_isAddressValid(JNIEnv *env, jclass clazz,
                                                  jstring address, jint networkType) {
    const char *_address = env->GetStringUTFChars(address, NULL);
    Coinevo::NetworkType _networkType = static_cast<Coinevo::NetworkType>(networkType);
    bool isValid = Coinevo::Wallet::addressValid(_address, _networkType);
    env->ReleaseStringUTFChars(address, _address);
    return static_cast<jboolean>(isValid);
}

JNIEXPORT jstring JNICALL
Java_io_wookey_coinevo_model_Wallet_getPaymentIdFromAddress(JNIEnv *env, jclass clazz,
                                                           jstring address,
                                                           jint networkType) {
    Coinevo::NetworkType _networkType = static_cast<Coinevo::NetworkType>(networkType);
    const char *_address = env->GetStringUTFChars(address, NULL);
    std::string payment_id = Coinevo::Wallet::paymentIdFromAddress(_address, _networkType);
    env->ReleaseStringUTFChars(address, _address);
    return env->NewStringUTF(payment_id.c_str());
}

JNIEXPORT jlong JNICALL
Java_io_wookey_coinevo_model_Wallet_getMaximumAllowedAmount(JNIEnv *env, jclass clazz) {
    return Coinevo::Wallet::maximumAllowedAmount();
}

JNIEXPORT void JNICALL
Java_io_wookey_coinevo_model_Wallet_startRefresh(JNIEnv *env, jobject instance) {
    Coinevo::Wallet *wallet = getHandle<Coinevo::Wallet>(env, instance);
    wallet->startRefresh();
}

JNIEXPORT void JNICALL
Java_io_wookey_coinevo_model_Wallet_pauseRefresh(JNIEnv *env, jobject instance) {
    Coinevo::Wallet *wallet = getHandle<Coinevo::Wallet>(env, instance);
    wallet->pauseRefresh();
}

JNIEXPORT jboolean JNICALL
Java_io_wookey_coinevo_model_Wallet_refresh(JNIEnv *env, jobject instance) {
    Coinevo::Wallet *wallet = getHandle<Coinevo::Wallet>(env, instance);
    return static_cast<jboolean>(wallet->refresh());
}

JNIEXPORT void JNICALL
Java_io_wookey_coinevo_model_Wallet_refreshAsync(JNIEnv *env, jobject instance) {
    Coinevo::Wallet *wallet = getHandle<Coinevo::Wallet>(env, instance);
    wallet->refreshAsync();
}
...
JNIEXPORT jlong JNICALL
Java_io_wookey_coinevo_model_Wallet_createTransactionJ(JNIEnv *env, jobject instance,
    ...
    const char *_payment_id = env->GetStringUTFChars(payment_id, NULL);
    Coinevo::PendingTransaction::Priority _priority =
            static_cast<Coinevo::PendingTransaction::Priority>(priority);
    Coinevo::Wallet *wallet = getHandle<Coinevo::Wallet>(env, instance);

    Coinevo::PendingTransaction *tx = wallet->createTransaction(_dst_addr, _payment_id,
...
JNIEXPORT jlong JNICALL
Java_io_wookey_coinevo_model_Wallet_createSweepTransaction(JNIEnv *env, jobject instance,
                                                          jstring dst_addr, jstring payment_id,
                                                          jint mixin_count,
                                                          jint priority,
                                                          jint accountIndex) {
    ...
    Coinevo::optional<uint64_t> empty;
...
JNIEXPORT jlong JNICALL
Java_io_wookey_coinevo_model_Wallet_createSweepUnmixableTransactionJ(JNIEnv *env,
                                                                    jobject instance) {
    Coinevo::Wallet *wallet = getHandle<Coinevo::Wallet>(env, instance);
    Coinevo::PendingTransaction *tx = wallet->createSweepUnmixableTransaction();
    return reinterpret_cast<jlong>(tx);
}
...
JNIEXPORT void JNICALL
Java_io_wookey_coinevo_model_Wallet_disposeTransaction(JNIEnv *env, jobject instance,
                                                      jobject pendingTransaction) {
    Coinevo::Wallet *wallet = getHandle<Coinevo::Wallet>(env, instance);
    Coinevo::PendingTransaction *_pendingTransaction =
            getHandle<Coinevo::PendingTransaction>(env, pendingTransaction);
...
JNIEXPORT jlong JNICALL
Java_io_wookey_coinevo_model_Wallet_getHistoryJ(JNIEnv *env, jobject instance) {
    Coinevo::Wallet *wallet = getHandle<Coinevo::Wallet>(env, instance);
...
JNIEXPORT jlong JNICALL
Java_io_wookey_coinevo_model_Wallet_setListenerJ(JNIEnv *env, jobject instance,
...
JNIEXPORT jint JNICALL
Java_io_wookey_coinevo_model_Wallet_getDefaultMixin(JNIEnv *env, jobject instance) {
    Coinevo::Wallet *wallet = getHandle<Coinevo::Wallet>(env, instance);
    return wallet->defaultMixin();
}

JNIEXPORT void JNICALL
Java_io_wookey_coinevo_model_Wallet_setDefaultMixin(JNIEnv *env, jobject instance, jint mixin) {
    Coinevo::Wallet *wallet = getHandle<Coinevo::Wallet>(env, instance);
    return wallet->setDefaultMixin(mixin);
}

JNIEXPORT jboolean JNICALL
Java_io_wookey_coinevo_model_Wallet_setUserNote(JNIEnv *env, jobject instance,
...
JNIEXPORT jstring JNICALL
Java_io_wookey_coinevo_model_Wallet_getUserNote(JNIEnv *env, jobject instance,
                                               jstring txid) {
...
JNIEXPORT jstring JNICALL
Java_io_wookey_coinevo_model_Wallet_getTxKey(JNIEnv *env, jobject instance,
                                            jstring txid) {
...
JNIEXPORT void JNICALL
Java_io_wookey_coinevo_model_Wallet_addAccount(JNIEnv *env, jobject instance,
                                              jstring label) {
...
JNIEXPORT jstring JNICALL
Java_io_wookey_coinevo_model_Wallet_getSubaddressLabel(JNIEnv *env, jobject instance,
...
JNIEXPORT void JNICALL
Java_io_wookey_coinevo_model_Wallet_setSubaddressLabel(JNIEnv *env, jobject instance,
                                                      jint accountIndex, jint addressIndex,
                                                      jstring label) {
...
JNIEXPORT jint JNICALL
Java_io_wookey_coinevo_model_Wallet_getNumAccounts(JNIEnv *env, jobject instance) {
    Coinevo::Wallet *wallet = getHandle<Coinevo::Wallet>(env, instance);
...
JNIEXPORT jint JNICALL
Java_io_wookey_coinevo_model_Wallet_getNumSubaddresses(JNIEnv *env, jobject instance,
                                                      jint accountIndex) {
    Coinevo::Wallet *wallet = getHandle<Coinevo::Wallet>(env, instance);
...
JNIEXPORT void JNICALL
Java_io_wookey_coinevo_model_Wallet_addSubaddress(JNIEnv *env, jobject instance,
                                                 jint accountIndex,
                                                 jstring label) {
...
// TransactionHistory
JNIEXPORT jint JNICALL
Java_io_wookey_coinevo_model_TransactionHistory_getCount(JNIEnv *env, jobject instance) {
    Coinevo::TransactionHistory *history = getHandle<Coinevo::TransactionHistory>(env,
                                                                                      instance);
...
jobject newTransferList(JNIEnv *env, Coinevo::TransactionInfo *info) {
    const std::vector<Coinevo::TransactionInfo::Transfer> &transfers = info->transfers();
    ...
    for (const Coinevo::TransactionInfo::Transfer &s: transfers) {
...
jobject newTransactionInfo(JNIEnv *env, Coinevo::TransactionInfo *info) {
    ...
    if (info->direction() == Coinevo::TransactionInfo::Direction_In)
...
jobject newSubaddressRow(JNIEnv *env, Coinevo::SubaddressRow *row) {
...
jobject cpp2java(JNIEnv *env, std::vector<Coinevo::TransactionInfo *> vector) {
    ...
    for (Coinevo::TransactionInfo *s: vector) {
...
jobject cpp2java2(JNIEnv *env, std::vector<Coinevo::SubaddressRow *> vector) {
    ...
    jobject arrayList = env->NewObject(class_ArrayList, java_util_ArrayList_, vector.size());
    for (Coinevo::SubaddressRow *s: vector) {
...
JNIEXPORT jobject JNICALL
Java_io_wookey_coinevo_model_Wallet_getSubaddresses(JNIEnv *env, jobject instance,
                                                   jint accountIndex) {

    Coinevo::Wallet *wallet = getHandle<Coinevo::Wallet>(env, instance);
//    size_t num = wallet->numSubaddresses(accountIndex);
    //wallet->subaddress()->getAll()[num]->getAddress().c_str()
    Coinevo::Subaddress *s = wallet->subaddress();
    s->refresh(accountIndex);
    std::vector<Coinevo::SubaddressRow *> v = s->getAll();
    return cpp2java2(env, v);
}

JNIEXPORT jobject JNICALL
Java_io_wookey_coinevo_model_TransactionHistory_refreshJ(JNIEnv *env, jobject instance) {
    Coinevo::TransactionHistory *history = getHandle<Coinevo::TransactionHistory>(env,
                                                                                      instance);
    history->refresh();
...
JNIEXPORT jint JNICALL
Java_io_wookey_coinevo_model_PendingTransaction_getStatusJ(JNIEnv *env, jobject instance) {
    Coinevo::PendingTransaction *tx = getHandle<Coinevo::PendingTransaction>(env, instance);
    return tx->status();
}

JNIEXPORT jstring JNICALL
Java_io_wookey_coinevo_model_PendingTransaction_getErrorString(JNIEnv *env, jobject instance) {
    Coinevo::PendingTransaction *tx = getHandle<Coinevo::PendingTransaction>(env, instance);
...
JNIEXPORT jboolean JNICALL
Java_io_wookey_coinevo_model_PendingTransaction_commit(JNIEnv *env, jobject instance,
...
JNIEXPORT jlong JNICALL
Java_io_wookey_coinevo_model_PendingTransaction_getAmount(JNIEnv *env, jobject instance) {
    Coinevo::PendingTransaction *tx = getHandle<Coinevo::PendingTransaction>(env, instance);
    return tx->amount();
}

JNIEXPORT jlong JNICALL
Java_io_wookey_coinevo_model_PendingTransaction_getDust(JNIEnv *env, jobject instance) {
    Coinevo::PendingTransaction *tx = getHandle<Coinevo::PendingTransaction>(env, instance);
    return tx->dust();
}

JNIEXPORT jlong JNICALL
Java_io_wookey_coinevo_model_PendingTransaction_getFee(JNIEnv *env, jobject instance) {
    Coinevo::PendingTransaction *tx = getHandle<Coinevo::PendingTransaction>(env, instance);
    return tx->fee();
}
...
JNIEXPORT jstring JNICALL
Java_io_wookey_coinevo_model_PendingTransaction_getFirstTxIdJ(JNIEnv *env, jobject instance) {
    Coinevo::PendingTransaction *tx = getHandle<Coinevo::PendingTransaction>(env, instance);
...
JNIEXPORT jlong JNICALL
Java_io_wookey_coinevo_model_PendingTransaction_getTxCount(JNIEnv *env, jobject instance) {
    Coinevo::PendingTransaction *tx = getHandle<Coinevo::PendingTransaction>(env, instance);
    return tx->txCount();
}


// these are all in Coinevo::Wallet - which I find wrong, so they are here!
...
JNIEXPORT void JNICALL
Java_io_wookey_coinevo_model_WalletManager_initLogger(JNIEnv *env, jclass clazz,
                                                     jstring argv0,
    ...
    Coinevo::Wallet::init(_argv0, _default_log_base_name);
...
JNIEXPORT void JNICALL
Java_io_wookey_coinevo_model_WalletManager_logDebug(JNIEnv *env, jclass clazz,
    ...
    Coinevo::Wallet::debug(_category, _message);
...
JNIEXPORT void JNICALL
Java_io_wookey_coinevo_model_WalletManager_logInfo(JNIEnv *env, jclass clazz,
                                                  jstring category, jstring message) {

    const char *_category = env->GetStringUTFChars(category, NULL);
    const char *_message = env->GetStringUTFChars(message, NULL);

    Coinevo::Wallet::info(_category, _message);
...
JNIEXPORT void JNICALL
Java_io_wookey_coinevo_model_WalletManager_logWarning(JNIEnv *env, jclass clazz,
                                                     jstring category, jstring message) {

    const char *_category = env->GetStringUTFChars(category, NULL);
    const char *_message = env->GetStringUTFChars(message, NULL);

    Coinevo::Wallet::warning(_category, _message);
...
JNIEXPORT void JNICALL
Java_io_wookey_coinevo_model_WalletManager_logError(JNIEnv *env, jclass clazz,
                                                   jstring category, jstring message) {

    const char *_category = env->GetStringUTFChars(category, NULL);
    const char *_message = env->GetStringUTFChars(message, NULL);

    Coinevo::Wallet::error(_category, _message);
...
JNIEXPORT void JNICALL
Java_io_wookey_coinevo_model_WalletManager_setLogLevel(JNIEnv *env, jclass clazz,
                                                      jint level) {
    Coinevo::WalletManagerFactory::setLogLevel(level);
}
